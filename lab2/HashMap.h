...
void AddItem(HashMap *hm, char *key,
             int value); // Функция добавления записы в хеш-таблицу
int GetValue(HashMap *hm, char *key); // Получаение значения по ключю
char *GetKeyByValue(HashMap *hm,
                    int value); // Функция получения ключа по значению (просто
                                // перебирает все ключи пока не найдет нужный)
char **
GetKeys(HashMap *hm); // Фунция получения списка всех ключей из хеш-таблицы
