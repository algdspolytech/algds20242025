...
// Создает новый узел дерева с указанием, является ли он листом, и его родителя
...
// Выполняет разделение полного узла при вставке
void split(B_node* parent_of_splited, int split_at_index) {
    B_node* full_node = parent_of_splited->children[split_at_index];
    B_node* new_node = createNode(full_node->is_leaf, parent_of_splited);

    if (!new_node) return;

    new_node->keys_count = T - 1;
    for (int j = 0; j < T - 1; ++j) {
        new_node->keys[j] = full_node->keys[j + T];
    }
    ...
    parent_of_splited->keys[split_at_index] = full_node->keys[T - 1];
    parent_of_splited->keys_count++;
}

// Вставляет элемент в неполный узел
...
// Освобождает память узлов дерева
...
// Освобождает все ресурсы дерева
...
// Возвращает итератор на начало обхода дерева
...
// Переходит к следующему элементу в итераторе
...
// Возвращает текущий ключ элемента на позиции итератора
