...
typedef struct {
  ...
  int nodes_length; // Текущее количество node'ов, нужно для ускорения/упрощения
                    // некоторых функций
...
int *ul_to_array(
    ul_list *list); // Преобразование всего содержимого ():№?*"%;:723854708
                    // списка в обычный массив(необходим для проведения тестов)
int ul_equal(int *list1, int *list2,
             int length); // Метод для сравнения двух обычных массивов
                          // (необходим для проведения тестов)
int ul_get_next(int number,
                ul_list *list); // Функция получения следующего элемента
