...
// Функция для вычисления значения выражения с учетом приоритета операций
long long evaluate_expression(const char *expr) {
    long long stack[100]; // Стек для промежуточных результатов
    int top = -1;
    long long num = 0;
    char last_op = '+';
    ...
    for (int i = 0; expr[i]; i++) {
        ...
        if (!isdigit(expr[i]) || expr[i + 1] == '\0') {
            if (expr[i + 1] == '\0' && isdigit(expr[i])) {
                i++; // Для обработки последней цифры
            }
            if (last_op == '*') {
                stack[top] *= num;
            } else if (last_op == '/') {
                if (num == 0) return LLONG_MAX; // Обработка деления на 0
                stack[top] /= num;
            } else {
...
int generate_expressions(char *a, int pos, char *expr, int expr_pos, long long b, char *result_expr) {
    int len = strlen(a);
    if (pos == len) {
        expr[expr_pos] = '\0'; // Завершаем выражение
        if (evaluate_expression(expr) == b) {
            strcpy(result_expr, expr); // Сохраняем результат
            return 1;
        }
        return 0;
    }
    ...
    int found = 0;
    expr[expr_pos] = a[pos]; // Добавляем цифру
    expr[expr_pos + 1] = '\0';
    found |= generate_expressions(a, pos + 1, expr, expr_pos + 1, b, result_expr);

    if (pos < len - 1) { // Добавляем операции
