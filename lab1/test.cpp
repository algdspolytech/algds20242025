#include "../LabA/LabA.cpp" // Здесь *.cpp а не *.h потому что, при сборке, линкер почему то не может найти нужные функции
    // в этом файле (такой костыль найден на StackOverflow, другого решения проблемы найти неудалось)
#include "pch.h"

// Тест если максимальный размер массива в одном узле равен 0, тогда список
// должен быть NULL так как такого списка быть не может
...
// в такой ситуации (т.к. максимальное число элементов в node равно 5) должен
// быть создан еще один node)
...
// Попробуем удалить из списка все элементы определенного node (тогда такой node
// должен быть полностью уничтожен)
...
// Попробуем удалить из списка элемент которого там нет (удаляем 13-ый элемент,
// а длинна списка 7)
