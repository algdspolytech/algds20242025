...
TEST(HashTableTest, InsertDuplicate) {
    ...
    ASSERT_TRUE(ht_insert(table, "key1", "value1"));
    ASSERT_TRUE(ht_insert(table, "key1", "value2")); // Îáíîâëåíèå
...
TEST(HashTableTest, Delete) {
    ...
    EXPECT_FALSE(ht_delete(table, "key3")); // Íåñóùåñòâóþùèé êëþ÷

    ht_destroy(table);
}

TEST(HashTableTest, CollisionHandling) {
    HashTable* table = ht_create(1); // Âñå ïîïàäåò â îäèí áàêåò äëÿ òåñòà êîëëèçèé
...
TEST(HashTableTest, MemoryAllocationFailure) {
    // Òåñò ìîæåò áûòü íåñòàáèëüíûì, òàê êàê çàâèñèò îò îêðóæåíèÿ
    HashTable* table = ht_create(0x7FFFFFFF); // Î÷åíü áîëüøàÿ òàáëèöà
