...
static unsigned long long hash_djb2(const char* s) {
...
static HashTable* ht_create(size_t cap) {
...
static void ht_insert(HashTable* ht, const char* key);
static void ht_delete(HashTable* ht, const char* key);
static int ht_contains(HashTable* ht, const char* key);
...
static void ht_rehash(HashTable* ht) {
    ...
    for (size_t i = 0; i < old_cap; i++) {
        if (old_table[i].state == USED) {
            unsigned long long h = hash_djb2(old_table[i].key);
...
static void ht_insert(HashTable* ht, const char* key) {
    ...
    unsigned long long h = hash_djb2(key);
...
int main(void) {
    ...
    while (scanf(" %c %511s", &cmd, buf) == 2) {
        if (cmd == 'a') {
            ...
            puts(ht_contains(ht, buf) ? "yes" : "no");
        }
    }
