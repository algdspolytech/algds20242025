...
void testLinkedQueue() {
    ...
    printf("Тесты для связного списка\n\n");
    // Test_no1
    ...
    // Test_no3
    printf("Тест 3: Декью из пустой очереди\n");
    printf("Dequeued: %d\n", dequeueLinked(q)); // Ожидаем -1

    // Test_no4
    printf("Тест 4: Добавление элементов после удаления\n");
    enqueueLinked(q, 40);
...
void testArrayQueue() {
    ...
    printf("Попытка поставить в очередь еще один элемент(80):\n");
    enqueueArray(q, 80); // Ожидаем сообщение об ошибке

    // Test_no9
    printf("Тест 9: Декью из пустой массивной очереди\n");
    while (!isEmpty(q)) {
        printf("Dequeued: %d\n", dequeueArray(q)); // Декьюим элементы до пустой очереди
    }
    printf("Dequeued: %d\n", dequeueArray(q)); // Ожидаем -1

    // Test_no10
    printf("Тест 10: Добавление элементов после опустошения очереди\n");
    enqueueArray(q, 90);
