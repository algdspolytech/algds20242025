...
split_t const _split = split;
insertNotFull_t const _insertNotFull = insertNotFull;
...
void split(B_node* parent_of_splited, int split_at_index) {
    if (!parent_of_splited || split_at_index < 0 ||
        split_at_index >= MAX_CHILDREN ||
        !parent_of_splited->children[split_at_index]) {
        return;
    }
...
void insertNotFull(B_node* node, int data) {
    ...
    if (node->is_leaf) {
        ...
        if (!node->children[i]) {
            node->children[i] = createNode(true, node);
            if (!node->children[i])
                return;
        }
...
void Insert(B_tree* tree, int data) {
    ...
    if (tree->root->keys_count == MAX_KEYS) {
        ...
        int child_idx_to_insert = (data > new_root->keys[0]) ? 1 : 0;
