...
#define TABLE_SIZE 1000003 // Большое простое число для уменьшения коллизий
...
// Оптимизированная хеш-функция 1
unsigned int hash1(const char* key) {
...
// Оптимизированная хеш-функция 2 (всегда нечетная)
unsigned int hash2(const char* key) {
    unsigned int hash = 5381;
    while (*key) {
        hash = ((hash << 5) + hash) ^ *key++;
    }
    return (hash % (TABLE_SIZE - 2)) + 1;  // Гарантированно нечетное
}
