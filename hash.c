...
// Õåø-ôóíêöèÿ äëÿ ñòğîê (djb2)
unsigned long hash_func(const char* str, int capacity) {
...
bool ht_insert(HashTable* table, const char* key, const char* value) {
    ...
    unsigned long index = hash_func(key, table->capacity);
    HashNode* node = table->buckets[index];

    // Ïğîâåğÿåì, ñóùåñòâóåò ëè óæå êëş÷
    while (node) {
        if (strcmp(node->key, key) == 0) {
            // Êëş÷ ñóùåñòâóåò, îáíîâëÿåì çíà÷åíèå
            free(node->value);
...
char* ht_search(HashTable* table, const char* key) {
    ...
    unsigned long index = hash_funct(key, table->capacity);
    HashNode* node = table->buckets[index];
