...
static B_node* init_node(bool is_leaf, B_node* parent) {
...
void split(B_node* parent_node, int index) {
    B_node* child_to_split = parent_node->children[index];
    ...
    for (int j = parent_node->keys_count; j > index; j--) {
        parent_node->children[j + 1] = parent_node->children[j];
    }
    parent_node->children[index + 1] = new_sibling;

    for (int j = parent_node->keys_count - 1; j >= index; j--) {
        parent_node->keys[j + 1] = parent_node->keys[j];
    }
    parent_node->keys[index] = child_to_split->keys[MIN_CHILDREN];
...
void Insert(B_tree* tree, int key) {
    ...
    if (current_root->keys_count == MAX_KEYS) {
        ...
        (*InsertNotFullPtr)(current_root, key);
    }
}
