...
int Test_setqueue_no1() {
    Queue queue;
    setqueue(&queue, 10);
    int status = 1;
    status *= ASSERT_EQ(queue.front, -1, "queue.front should be -1", "Test_setqueue_no1");
    status *= ASSERT_EQ(queue.rear, -1, "queue.rear should be -1", "Test_setqueue_no1");
    status *= ASSERT_EQ(queue.capacity, 10, "queue.capacity should be 10", "Test_setqueue_no1");
    freeQueue(&queue);
    print_result_testing(status, "Test_setqueue_no1");
    return status;
}
...
int main() {
    ...
    result_testing *= Test_setqueue_no1();
    ...
    result_testing *= Test_mainfunct_no6();
