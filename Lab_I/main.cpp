...
static B_node* creatingNode(bool is_leaf, B_node* parent) {
...
void split(B_node* parent, int i) {
    ...
    B_node* new_child = creatingNode(full_child->is_leaf, parent);
    if (!new_child) return; 

    new_child->keys_count = MIN_CHILDREN;

    // Копирование ключей и потомков из полного узла в новый
...
void Insert(B_tree* tree, int data) {
    ...
    if (!tree->root) {
        ...
        tree->root = creatingNode(true, NULL);
        if (!tree->root) return;
    ...
    if (root->keys_count == MAX_KEYS) {
        ...
        B_node* new_root = creatingNode(false, NULL);
        if (!new_root) return;
...
// Инициализация итератора для обхода дерева
...
int Next(B_tree_iterator* iter) {
    ...
    // Подъем по дереву к следующему ключу
    B_node* cur = node;
    while (cur->parent) {
