...
ul_list *ul_del_item(ul_list *list, int index) {
  ...
  while (node != NULL) {
    if (index < node->length) {
      ...
      // Если node оказался пустой то его надо удалить, т.е. связать предыдущий
      // и следующий node'ы вместе
      if (node->length == 0) {
        if (last_node != NULL)
...
// Преобразование всего содержимого развернутого связного списка в обычный
// массив(необходим для проведения тестов)
