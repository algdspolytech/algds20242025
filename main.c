...
Polynom parsePolynom(const char* polynomialString) {
    ...
    while ((token = strtok_r(rest, "+", &rest))) {
        poly.size++;
        poly.monomials = (Monom*)realloc(poly.monomials, poly.size * sizeof(Monom));
        char* coeffStr;
        char* expStr;

        if ((coeffStr = strtok(token, "*x^"))) {
            if (strtok(NULL, "*x^") == NULL) {
                poly.monomials[poly.size - 1].coeff = atoll(coeffStr);
                poly.monomials[poly.size - 1].power = 0;
            }
            else {
                expStr = strtok(NULL, "*x^");
                poly.monomials[poly.size - 1].coeff = atoll(coeffStr);
                poly.monomials[poly.size - 1].power = atoi(expStr);
            }
        }
    }
    return poly;
...
char* formatPolynom(Polynom p) {
    ...
    for (int i = 0; i < p.size; i++) {
        ...
        if (power > 0) {
            if (coeff != -1 && strlen(monomialStr) > 0) {
                strcat(monomialStr, "*");
            }
            if (power == 1) {
...
Polynom addPolynoms(Polynom p1, Polynom p2) {
    ...
    while (i < p1.size && j < p2.size) {
        if (p1.monomials[i].power < p2.monomials[j].power) {
            result.size++;
            result.monomials = realloc(result.monomials, result.size * sizeof(Monom));
...
int main() {
    ...
    Polynom p1 = parsePolynom(line1);
    Polynom p2 = parsePolynom(line2);
    ...
    fprintf(outputFile, "%s\n", formatPolynom(product));
    fprintf(outputFile, "%s\n", formatPolynom(derivative));
