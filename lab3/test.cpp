...
#include "../Lab_D/main.h"
#include <fstream>
#include <string>

// Тест 1: Чтение задач из корректного файла (предполагается, что input.txt уже существует и содержит корректные данные)
...
// Тест 2: Чтение задач из файла с некорректными данными (предполагается, что input.txt содержит некорректные данные)
...
// Тест 3: Запись корректного расписания в файл (предполагается, что output.txt используется для проверки результата)
TEST(TaskSchedulerTest, WriteSchedule_ValidOutput_no3) {
    ...
    // Проверка содержимого существующего файла "output.txt"
    std::ifstream file("output_test1.txt");
    std::string content((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
    EXPECT_EQ(content, "1 2 3 \n");
}

// Тест 4: Запись пустого расписания в файл (предполагается, что output.txt используется для проверки результата)
...
// Тест 5: Проверка с пустым множеством задач
...
// Тест 6: Задачи укладываются в ограничение K
...
// Тест 9: Одна задача, которая превышает K
...
// Тест 10: Большое множество задач
