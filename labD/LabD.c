...
int main() {
    ...
    //Unit tests
    test_appendV_empty_list(); //проверка добавления вершины в пустой список
    test_appendV_nonempty_list(); //проверка добавления вершины в не пустой список
    test_appendG_empty_list(); //проверка добавления в пустой список
    test_appendG_nonempty_list(); //проверка добавления в не пустой список
    test_appendG_null(); //проверка добавления null в список
    test_checkadjacentvertex_true(); //проверка смежности двух смежных вершин
    test_checkadjacentvertex_false(); //проверка смежности двух не смежных вершин
    test_cover_true(); //проверка вывода, когда существует V'
    test_cover_false(); //проверка вывода, когда не существует V'
    test_cover_one_element_graph(); //проверка вывода, когда в графе 1 вершина
    return 0;
}
//Unit tests
